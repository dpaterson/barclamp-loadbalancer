...
["quantum", "cinder", "glance", "keystone", "swift", "nova"].each do |prop|
  ...
  when "quantum"
    ...
    services["quantum"]=fill_instance(port_enum, env_filter)
  when "cinder"
    ...
    services["cinder"]=fill_instance(port_enum, env_filter)
  when "glance"
    env_filter = "roles:glance-server AND roles:glance-config-#{node[:loadbalancer][:glance_instance]}"
    port_enum = "n[:glance][:api][:bind_port]"
    services["glance"]=fill_instance(port_enum, env_filter)
  when "keystone"
    env_filter = "roles:keystone-server AND roles:keystone-config-#{node[:loadbalancer][:keystone_instance]}"
    services["keystone-api"]=fill_instance("n[:keystone][:api][:api_port]", env_filter)
    services["keystone-admin"]=fill_instance("n[:keystone][:api][:admin_port]", env_filter)
  ...
  when "swift"
    ...
    port_enum = "8080"
    services["swift-proxy"]=fill_instance(port_enum, env_filter)
  when "nova"
    env_filter = "roles:nova-multi-controller AND roles:nova-config-#{node[:loadbalancer][:nova_instance]}"
    port_enum = "8774"
    services["nova-api"]=fill_instance(port_enum, env_filter)
  end
end
